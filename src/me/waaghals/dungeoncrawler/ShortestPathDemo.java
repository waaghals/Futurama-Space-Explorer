...
public class ShortestPathDemo extends JPanel {
	 ...
	 */
	Graph<String,Number> getGraph(int numRooms, int maxNumPaths) {

		Graph<String,Number> g =
			new EppsteinPowerLawGenerator<String,Number>(
					new GraphFactory(), new VertexFactory(), new EdgeFactory(), numRooms, maxNumPaths, numRooms* maxNumPaths).create();
		
		//Nominees Sets
		Set<String> removeVertex = new HashSet<String>();
		Set<String> removeEdge = new HashSet<String>();
		...
		//Remove all nominees
		for(String v : removeEdge) {
			Collection<Number> edges = g.getOutEdges(v);
			while(g.degree(v) > 4){
				g.removeEdge(edges.iterator().next()); //Remove the first edge
			}
		}
