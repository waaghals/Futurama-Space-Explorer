package me.waaghals.dungeoncrawler.bugfix;
 ...
 * 
 */
public class HackedAudioPlayer extends JavaStreamingAudioPlayer {
	...
	private AudioFormat defaultFormat = new AudioFormat(8000f, 16, 1, true,
			true);
	...
	/**
	 * controls the number of bytes of audio to write to the buffer for each
	 * call to write()
	 */
	...
	public HackedAudioPlayer() {
		...
		openFailDelayMs = Utilities.getLong(
				...
				.longValue();
		totalOpenFailDelayMs = Utilities
				.getLong(
						"com.sun.speech.freetts.audio.AudioPlayer.totalOpenFailDelayMs",
						0L).longValue();
		audioMetrics = Utilities
				.getBoolean("com.sun.speech.freetts.audio.AudioPlayer.showAudioMetrics");
	...
	private synchronized void openLine(AudioFormat format) {
		...
		do {
			try {
				...
				System.err.println("LINE UNAVAILABLE: " + "Format is "
						+ currentFormat);
				try {
	 ...
	 * actually opens the audio line. Since this is a streaming audio player,
	 * the <code>size</code> parameter has no meaning and effect at all, so any
	 * value can be used. Audio data for a single utterance should be grouped
	 * between begin/end pairs.
	 * 
	 * @param size
	 *            supposedly the size of data between now and the end, but since
	 *            this is a streaming audio player, this parameter has no
	 *            meaning and effect at all
	 */
	 ...
	 *         stopped
	 * 
	 *         [[[ WORKAROUND TODO The javax.sound.sampled drain is almost
	 *         working properly. On linux, there is still a little bit of sound
	 *         that needs to go out, even after drain is called. Thus, the
	 *         drainDelay. We wait for a few hundred milliseconds while the data
	...
	public boolean write(byte[] bytes, int offset, int size) {
		...
		debugPrint(" au write " + bytesRemaining + " pos "
				+ line.getMicrosecondPosition() + " avail " + line.available()
		...
		while (bytesRemaining > 0 && !isCancelled()) {
			...
			synchronized (lineLock) {
				...
				if (bytesWritten != bytesWritten) {
					debugPrint("RETRY! bw" + bytesWritten + " br "
							+ bytesRemaining);
				}
			...
			debugPrint("   wrote " + " cur " + curIndex + " br "
					+ bytesRemaining + " bw " + bytesWritten);
	...
	/**
	 * Waits for resume. If this audio player is paused waits for the player to
	 * be resumed. Returns if resumed, cancelled or shutdown.
	 * 
	 ...
	 * variable should be within a synchronized block such as this to ensure
	 * that access is coherent.
	 * 
	 ...
	 * within a synchronized block such as this to ensure that access is
	 * coherent.
	 * 
